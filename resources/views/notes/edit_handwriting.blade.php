        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="rounded-[20px] border-2 border-black/10 bg-white shadow-xl">
                <div class="p-6 md:p-8">
                    ...
                    <h1 class="text-2xl md:text-3xl font-bold text-black/90 mb-6">{{ __('Редактирование рукописной заметки') }}</h1>

                    {{-- !!! ИЗМЕНЕНИЕ 2: ФОРМА - action и method !!! --}}
                    <form id="handwriting-form" method="POST" action="{{ route('notes.update.handwriting', [$track, $note]) }}">
                        @csrf
                        @method('PATCH')
                        ...
                        {{-- Панель инструментов (остается прежней) --}}
...
@section('scripts')
    ...
    <script>
        ...
        function initCanvas() {
            ...
            // !!! ИЗМЕНЕНИЕ 3: ЗАГРУЗКА СУЩЕСТВУЮЩЕГО КОНТЕНТА !!!
            const existingContentJson = @json($note->content ?? 'null');

            if (existingContentJson && existingContentJson !== 'null') {
                try {
                    // Fabric.js загружает JSON-объект и перерисовывает холст
                    canvas.loadFromJSON(existingContentJson, () => {
                        canvas.renderAll();
                        // После загрузки, мы позволим коду ниже форсировать режим 'pen'
                    });
                } catch (e) {
            ...
            const setPenMode = () => {
                ...
                canvas.skipTargetFind = true; // Отключаем поиск объектов
            ...
            const setEraserMode = () => {
                ...
                canvas.skipTargetFind = false; // ВКЛЮЧАЕМ поиск объектов для Ластика
                canvas.selection = false;
            ...
            // Этот клик сработает и после загрузки контента, что обеспечит правильные режимы кисти.
            ...
            // Корректировка размеров при ресайзе окна (остается прежней)
